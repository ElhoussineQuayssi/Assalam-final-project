...
export async function getStats() {
  try {
    ...
    const totalBlogs = totalBlogsData?.length || 0;
    const newBlogs = totalBlogsData?.filter(blog =>
      new Date(blog.created_at) >= thirtyDaysAgo
    ).length || 0;

    // Get total messages and new messages
    const { data: totalMessagesData, error: messagesError } = await supabaseAdmin
      .from('messages')
      .select('id, created_at', { count: 'exact' });

    if (messagesError) throw messagesError;

    const totalMessages = totalMessagesData?.length || 0;
    const newMessages = totalMessagesData?.filter(message =>
    ...
    const totalViews = viewsData?.reduce((sum, blog) => sum + (blog.views || 0), 0) || 0;
    const previousViews = viewsData?.filter(blog =>
      new Date(blog.created_at) < thirtyDaysAgo
    ).reduce((sum, blog) => sum + (blog.views || 0), 0) || 0;

    const currentViews = viewsData?.filter(blog =>
      new Date(blog.created_at) >= thirtyDaysAgo
    ).reduce((sum, blog) => sum + (blog.views || 0), 0) || 0;

    // Get total projects and new projects (last 30 days)
    const { data: totalProjectsData, error: projectsError } = await supabaseAdmin
      .from('projects')
      .select('id, created_at', { count: 'exact' });

    if (projectsError) throw projectsError;

    const totalProjects = totalProjectsData?.length || 0;
    const newProjects = totalProjectsData?.filter(project =>
      new Date(project.created_at) >= thirtyDaysAgo
    ).length || 0;

    // Get total beneficiaries from projects
    const { data: beneficiariesData, error: beneficiariesError } = await supabaseAdmin
      .from('projects')
    ...
    const totalBeneficiaries = beneficiariesData?.reduce((sum, project) => {
    ...
    // Get recent messages
    const { data: recentMessagesRaw, error: recentMessagesError } = await supabaseAdmin
      .from('messages')
    ...
    const recentMessages = recentMessagesRaw?.map(msg => ({
      ...
      excerpt: msg.message.length > 50 ? msg.message.substring(0, 50) + '...' : msg.message,
      type: msg.type,
      date: msg.created_at
    })) || [];

    // Get recent projects
    const { data: recentProjects, error: recentProjectsError } = await supabaseAdmin
      .from('projects')
    ...
    const newMessagesPercent = Math.round((newMessages / totalMessages) * 100) || 0;
    const newProjectsPercent = Math.round((newProjects / totalProjects) * 100) || 0;
    ...
    return {
      ...
      adminsCount: totalAdmins || 0, // For backward compatibility
      viewsChangePercent,
...
export async function getProjects() {
  try {
    const { data: projects, error } = await supabaseAdmin
      .from('projects')
      .select('id, slug, title, excerpt, image, categories, start_date, location, people_helped, status, content, goals, gallery, created_at, updated_at')
      .order('created_at', { ascending: false });
