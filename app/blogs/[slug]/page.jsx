    ...
    <main style={{ backgroundColor: BACKGROUND }}>
      <Container className="py-16">
        <article>
          ...
          <SectionWithBackground variant="blue" className="text-center mb-12 max-w-4xl mx-auto rounded-xl shadow-lg">
            <div className="mb-6">
            ...
            <div className="mb-6">
              <Link
                ...
                style={{ backgroundColor: `${PRIMARY_LIGHT}B3`, color: DARK_TEXT, border: `1px solid ${ACCENT}80` }}
              >
                {blog.category}
              </Link>
            </div>
            <h1 className="text-4xl md:text-5xl font-bold mb-6" style={{ color: ACCENT }}>{blog.title}</h1>
            <p className="text-xl mb-8" style={{ color: `${DARK_TEXT}D9` }}>{blog.excerpt}</p>

            <div className="flex flex-wrap justify-center gap-6 text-sm" style={{ color: `${DARK_TEXT}B3` }}>
              <div className="flex items-center">
                <Calendar className="w-4 h-4 mr-2" style={{ color: ACCENT }} />
                <span>Publié le {formatDate(blog.createdAt)}</span> {/* Enhanced Content */}
              </div>
              <div className="flex items-center">
                <Clock className="w-4 h-4 mr-2" style={{ color: ACCENT }} />
                <span>Temps de lecture : {blog.readTime || 5} min</span> {/* Enhanced Content */}
              </div>
              <div className="flex items-center">
                <User className="w-4 h-4 mr-2" style={{ color: ACCENT }} />
                <span>Par {blog.author?.name || "l'Équipe Assalam"}</span> {/* Enhanced Content */}
              </div>
            </div>
          ...
          <div className="grid md:grid-cols-4 gap-12">
            ...
            <div className="hidden md:block">
              <div className="sticky top-12 space-y-8">
                <div>
                  <h4 className="text-lg font-semibold mb-4" style={{ color: ACCENT }}>Partager ce Récit de Solidarité</h4> {/* Enhanced Content */}
                ...
                <div>
                  <h4 className="text-lg font-semibold mb-4" style={{ color: ACCENT }}>Domaines d'Action</h4> {/* Enhanced Content */}
            ...
            <div className="md:col-span-2">
              <div className="bg-white rounded-xl shadow-2xl p-8 border border-gray-100 relative overflow-hidden card-lift">
                <div className="relative z-10">
                  ...
                  {blog.tags && (
                    <div className="mt-12 flex flex-wrap gap-2">
                      {blog.tags.split(',').map((tag, idx) => (
                        ...
                        >
                          <Tag className="inline w-3 h-3 mr-1" style={{ color: ACCENT }} />
                          {tag.trim()}
                        </Link>
                      ))}
                    </div>
                  ...
                  {blog.author && (
                    <div className="mt-12 p-6 rounded-xl shadow-md" style={{ backgroundColor: PRIMARY_LIGHT, borderLeft: `4px solid ${ACCENT}` }}>
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <h3 className="font-bold text-lg" style={{ color: DARK_TEXT }}>{blog.author.name}</h3>
                          <p className="text-sm" style={{ color: ACCENT }}>{blog.author.role}</p>
                        </div>
                      </div>
                      <p className="mt-4 text-sm" style={{ color: `${DARK_TEXT}D9` }}>{blog.author.bio}</p>
                    </div>
                  )}

                  {/* Mobile Share - Refactored for inline colors */}
                  <div className="mt-12 md:hidden">
                    <h4 className="text-lg font-semibold mb-4" style={{ color: DARK_TEXT }}>Partager cet article</h4>
                    <div className="flex gap-4">
                        {['facebook', 'twitter', 'linkedin', 'instagram'].map(platform => {
                            const Icon = platform === 'facebook' ? Facebook : platform === 'twitter' ? Twitter : platform === 'linkedin' ? Linkedin : Instagram;
                            const color = platform === 'facebook' ? '#1877F2' : platform === 'twitter' ? '#1DA1F2' : platform === 'linkedin' ? '#0A66C2' : '#E4405F';
                            return (
                                <a
                                    key={platform}
                                    href={(new ShareButton({ platform, slug, title: blog.title })).props.href} // Accessing URL from a temporary instance
                                    target="_blank"
            ...
            <div>
              <div className="sticky top-12">
                <h4 className="text-lg font-semibold mb-6" style={{ color: ACCENT }}>Plus de Récits d'Espoir et d'Impact</h4> {/* Enhanced Content */}
                <div className="space-y-6">
                  {relatedBlogs.map((post, idx) => (
                    <Link href={`/blogs/${post.slug}`} key={idx} className="block">
                      <ContentCard
                        title={post.title}
