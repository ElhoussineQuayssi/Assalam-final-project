...
const ImageTextSection = ({ title, content, imageSrc, imageAlt, layout = "image-right" }) => {
  const isImageLeft = layout === 'image-left';

  return (
    <div className="max-w-7xl mx-auto grid md:grid-cols-2 gap-12 items-center py-12 scroll-reveal">
      {/* Image Block */}
      <div className={`relative h-96 rounded-xl overflow-hidden shadow-2xl ${isImageLeft ? 'md:order-1' : 'md:order-2'}`}>
        <Image
          src={imageSrc}
          alt={imageAlt}
          fill
        ...
        />
        <div className="absolute inset-0 border-b-8 border-opacity-70 pointer-events-none" style={{ borderColor: '#6495ED' }}></div>
      </div>
