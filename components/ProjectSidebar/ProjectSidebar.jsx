...
const ProjectSidebar = ({ projectTitle, targetAudience, facilities }) => (
  <aside className="md:col-span-1">
    ...
    >
      <h3 className="text-2xl font-bold mb-4" style={{ color: '#333333' }}>Participer à la Réussite de {projectTitle}</h3>
      {/* FIX: Replaced text-gray-700 with MUTED_TEXT */}
      <p className="mb-6" style={{ color: '#767676' }}>
        Votre soutien est la clé qui assure la pérennité et l'élargissement de cette œuvre de bienfaisance.
      </p>
      ...
      <div className="mt-8 pt-6 border-t space-y-4" style={{ borderColor: '#B0E0E6' }}>
        <h4 className="font-semibold text-lg" style={{ color: '#6495ED' }}>Les Piliers du Projet</h4>
        {targetAudience.length > 0 && (
          <div className="flex items-center gap-3" style={{ color: '#767676' }}>
            <Users className="h-5 w-5 flex-shrink-0" style={{ color: '#6495ED' }} />
            <span>**Communautés Servies:** {targetAudience.join(', ')}</span>
          </div>
        )}
        {facilities.length > 0 && (
          <div className="flex items-center gap-3" style={{ color: '#767676' }}>
            <MapPin className="h-5 w-5 flex-shrink-0" style={{ color: '#6495ED' }} />
            <span>**Réalisations Concrètes:** {facilities.join(', ')}</span>
          </div>
        )}
        <div className="flex items-center gap-3" style={{ color: '#767676' }}>
          <FileText className="h-5 w-5 flex-shrink-0" style={{ color: '#6495ED' }} />
          <Link href="#documentation" className="hover:underline font-semibold transition-colors duration-200" style={{ color: '#6495ED' }}>
